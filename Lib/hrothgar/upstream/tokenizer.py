...
class CNNEncoder(nn.Module):
    def __init__(
        ...
        for i_level in range(self.num_resolutions):
            ...
            if i_level != self.num_resolutions - 1:
                down_block.downsample = ConvBlock(
                    curr_in_ch,
                    curr_in_ch,
                    kernel_size=3,
                    stride=1,
        ...
        self.conv_out = ConvBlock(
            curr_in_ch, z_channels, kernel_size=3, stride=1, padding=1
        )
...
class Tokenizer(nn.Module):
    def __init__(self, config: TokenizerModelArgs):
        ...
        self.vit_encoder = ViTEncoder(
            ...
            config.vit_dim,
            config.vit_depth,
            config.vit_heads_num,
            config.vit_mlp_dim,
            config.vit_heads_dim,
        )
