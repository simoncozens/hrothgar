...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, 
                 ...
                 bias=True,upsample=False,downsample=False,dropout=0.0
                 ):
        super().__init__()
        assert not(upsample and downsample), "Cannot upsample and downsample at the same time"
        if kernel_size ==1 :
        ...
        self.conv = w_norm(nn.Conv2d(in_channels, out_channels, kernel_size, stride, bias=bias))
...
class ResBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=1, 
                 norm='none', w_norm='none', activ='relu', pad_type='zero',
                 bias=True,upsample=False,downsample=False,dropout=0.0, scale_var = False):
        super().__init__()
        assert not(upsample and downsample), "Cannot upsample and downsample at the same time"
        ...
        self.conv1  =ConvBlock(in_channels,out_channels,kernel_size,1,padding,
                               norm,w_norm,activ,pad_type,upsample=upsample,dropout=dropout,)
        self.conv2  =ConvBlock(out_channels,out_channels,kernel_size,1,padding,
                                norm,w_norm,activ,pad_type,dropout=dropout)
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        w_ = torch.bmm(q,k)     # [b,h*w,h*w]    w[b,i,j]=sum_c q[b,i,c]k[b,c,j] => i's attention on j
        w_ = w_ * (int(c)**(-0.5))
