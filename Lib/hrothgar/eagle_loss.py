...
class EagleLoss(nn.Module):
    ...
    def __init__(self, patch_size, cpu=False, cutoff=0.5):
        ...
        self.kernel_x = nn.Parameter(
            torch.tensor(kernel_values, dtype=torch.float32)
            .unsqueeze(0)
            .unsqueeze(0)
            .to(self.device),
            requires_grad=False,
        ...
        self.unfold = nn.Unfold(kernel_size=self.patch_size, stride=self.patch_size).to(
            self.device
        )
    ...
    def forward(self, output, target):
        ...
        if output.size(1) != 1 or target.size(1) != 1:
            raise ValueError(
                "Input 'output' and 'target' should be either RGB (3 channels) or grayscale (1 channel)"
            )
        ...
        eagle_loss = self.calculate_patch_loss(
            gx_output, gx_target
    ...
    def calculate_patch_loss(self, output_gradient, target_gradient):
        ...
        output_patches = self.unfold(output_gradient).view(
            batch_size, num_channels, patch_size_squared, -1
        )
        target_patches = self.unfold(target_gradient).view(
            batch_size, num_channels, patch_size_squared, -1
        )
        ...
        shape0, shape1 = (
            output_gradient.shape[-2] // self.patch_size,
            output_gradient.shape[-1] // self.patch_size,
        )
    ...
    def gaussian_highpass_weights2d(self, size):
        freq_x = (
            torch.fft.fftfreq(size[0]).reshape(-1, 1).repeat(1, size[1]).to(self.device)
        )
        freq_y = (
            torch.fft.fftfreq(size[1]).reshape(1, -1).repeat(size[0], 1).to(self.device)
        )
