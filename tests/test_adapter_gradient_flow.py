...
def test_adapter_gradient_flow_alignment_loss():
    ...
    visual_style_tokens = torch.randn(
        batch_size, style_token_count, style_token_dim, requires_grad=True
    )
    ...
    for name, param in adapter.named_parameters():
        if param.grad is not None:
            adapter_params_have_grads.append(
                (name, param.grad.abs().max().item() > 1e-8)
            )
    ...
    assert (
        style_out_grad.abs().max().item() > 1e-8
